...
mod tests {
    ...
    fn exact_v1_through_v8_manifests_match_shared_contracts() {
        ...
        let v4 = execute_fixture(SCHEMA_V4_SQL);
        assert_eq!(
            schema_manifest(&v4).unwrap(),
            expected_manifest(SchemaVersion::V4).unwrap()
        );
        validate_canonical_schema(&v4, SchemaVersion::V4).unwrap();
        let v5 = execute_fixture(SCHEMA_V5_SQL);
        assert_eq!(
            schema_manifest(&v5).unwrap(),
        ...
        let v8 = execute_fixture(SCHEMA_V8_SQL);
        assert_eq!(
            schema_manifest(&v8).unwrap(),
