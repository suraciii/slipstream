...
Prefixing every old Location preserves same-directory RAW/JPEG pairing within the former Folder. The subsequent scan discovers supported files in sibling directories as new Original Files and Photos. It rebuilds derived File Location navigation and does not create Albums from directories.
...
SQLite schema version 4 introduced the stable identity fence. A v3-to-v4 migration preserves every row and existing ID in one admitted `BEGIN IMMEDIATE` transaction. Canonical writable state is now schema version 5, whose Album terminology migration preserves that identity fence and every user-owned value. Older binaries must reject newer state.

Expansion is admitted only against canonical v5 state and a verified pre-expansion v5 backup. Its one transaction changes the admitted Folder binding, persisted Locations, location-derived ordering values, and rebuildable derived facts. It must not change user-owned state.
...
Rollback across the v3-to-v4 identity migration stops the newer process, restores the verified pre-migration v3 backup, restores the prior Library Folder configuration, and starts the compatible v3 image. Rollback across the v4-to-v5 Album migration restores its verified v4 backup and compatible v4 image. There is no in-place down migration. Rollback of an expansion while remaining on v5 restores the verified pre-expansion v5 backup and prior Library Folder.
...
- rollback restores the correct verified v3, v4, or v5 backup and prior Library Folder; and
