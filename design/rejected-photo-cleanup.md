...
The running server serves browsing from one in-memory published Library. A confirmed removal or restore patches that publication in place — the removal marker and the derived Original Folder index — so a source opened immediately afterwards already excludes or re-admits the Photo. The owning SQLite write and this patch are one critical section: a reader that serializes on the publication lock sees either the whole state before the commit or the whole state after its effect, never the commit without it. A read that does not serialize on that lock linearizes at the published snapshot, which the patch replaces under the snapshot's own write lock, so it reads one whole state or the other and never a mixture. The patch completes before the response, so a client that has been told the outcome never reads the state it replaced. A window of a Snapshot that was opened before the removal is not answered with a page that presents the removed Photo, and never with a page shorter than the position it names: the Snapshot is reported as expired instead, and the browser reopens the source and reads the Library as it now is. A rescan publishes a replacement that carries the persisted markers unchanged.
